...
void semantic_error(int line, char* msg1, char* msg2) {
	printf("\033[47;31mline %d\033[0m   %s %s \n", line, msg1, msg2);
}
...
void prn_symbol() {
	...
	printf("\033[44;32m                                       Symbol Table                                            \033[0m\n");
	printf("\t%s\t\t%s\t\t%s\t\t%s\t\t%s\t\t%s\t\n", "变量名",  "别名", "层号", "类型","标记","偏移量");


	for (i = 0; i < symbolTable.index; ++i) {
		if (!strcmp(symbolTable.symbols[i].name, "Null"))
			continue;
...
int fill_Temp(char* name, int level, int type, char flag, int offset) {
	strcpy(symbolTable.symbols[symbolTable.index].name, "temp");
	...
	if (T)
	{
		...
		case DIV:  
			...
			if (T->ptr[0]->type == CHAR) {
				semantic_error(T->pos, T->ptr[0]->type_id, "变量不能参与运算");
				break;
			}
			else if (T->ptr[1]->type == CHAR) {
				semantic_error(T->pos, T->ptr[1]->type_id, "变量不能参与运算");
				break;
			}
		...
		case AUTOADD:
			if (T->ptr[0]) {
				...
				if (T->ptr[0]->type != INT) {
					semantic_error(T->pos, "", "自增语句不为int");
					break;
				}
				Exp(T->ptr[0]);
				T->type = T->ptr[0]->type;
		...
		case AUTOSUB:
			if(T->ptr[0]) {
				...
				if (T->ptr[0]->type != INT) {
					semantic_error(T->pos, "", "自减语句不为int");
					break;
				}
				Exp(T->ptr[0]);
				T->type = T->ptr[0]->type;
		...
		case COMMOD:
			...
			if (T->ptr[0]->type == CHAR) {
				semantic_error(T->pos, T->ptr[0]->type_id, "不能参与复合赋值运算");
				break;
			}
			if (T->ptr[1]->type == CHAR) {
				semantic_error(T->pos, "", "不能参与复合赋值运算");
				break;
			}
			T->ptr[0]->offset = T->offset;
			Exp(T->ptr[0]);
